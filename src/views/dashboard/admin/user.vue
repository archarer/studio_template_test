  ...
  <div class="app-container" >
    ...
    <div class="filter-container">
      <el-input size="small" style="width: 200px;" class="filter-item" :placeholder="'账号'" v-model="readOptions.Account.Value">
       ...
       <el-input size="small" style="width: 200px;" class="filter-item" :placeholder="'用户名'" v-model="readOptions.UserName.Value">
      </el-input>
    </div>

    <el-table  :data="tableDataInfo.Data"  row-key="UserID"  v-loading="listLoading" ref="DataTable"  highlight-current-row  @sort-change="handleSort"  @select-all="selectAllRow"    @row-contextmenu="rowContextmenu"  border fit 
      style="width: 100%;"   >
      <el-table-column
      type="selection"
      width="35" >
      ...
      <el-table-column width="100px" align="center" label="直属上级">
        <template slot-scope="scope">
          <span style='color:red;'>{{scope.row.ChiefUserName}}</span>
        </template>
      </el-table-column>
        ...
        <el-aside width="400px">
          <el-form :rules="rules" ref="editForm" :model="rowData"  label-width="70px" style='width: 300px; margin-left:50px;'>
                ...
                <el-form-item label="密码" prop="Password">
                  ...
                  <el-switch  v-model="rowData.ChangePw" :active-value="true" :inactive-value="false" ></el-switch><el-tag v-if='rowData.UserID==""'>默认777777</el-tag>
                </el-form-item>
    ...
    <el-dialog title="已存在当前账户，是否确认该用户信息" :visible.sync="dialogBaseUserInfoVisible" width="30%" :append-to-body="true" :close-on-click-modal="false">
      <el-container>
...
export default {
  ...
  methods: {
    ...
    saveUserInfo() {
      this.$refs['editForm'].validate((valid) => {
        if (valid) {
          ...
          if (this.rowData.UserName === '' && this.baseUserInfo.UserName !== '') {
            this.rowData.UserName = this.baseUserInfo.UserName
          }
          if (this.rowData.NickName === '' && this.baseUserInfo.NickName !== '') {
            this.rowData.NickName = this.baseUserInfo.NickName
          }
          if (this.rowData.JobNo === '' && this.baseUserInfo.JobNo !== '') {
            this.rowData.JobNo = this.baseUserInfo.JobNo
          }
          if (this.rowData.Avatar === '' && this.baseUserInfo.Avatar !== '') {
            this.rowData.Avatar = this.baseUserInfo.Avatar
          }
          const postData = this.$qs.stringify(this.rowData)
          console.info(this.baseUserInfo)
    ...
    saveInfo() {
      ...
      if (this.manageProductID && this.manageProductID > 0 && this.rowData.UserID === '') {
        request({
    ...
    initReadOptions() {
      this.readOptions = {
        ...
        UserName: this.setQueryValue(this.queryMethod.Contains, this.queryDataType.String, ''),
